...
namespace NextMeeting.Helpers
{
    public class ImageHelper
    {
        ...
        public static string GetFileExtensionFromDocumentType(string typeDoc)
        {
            switch (typeDoc.ToLowerInvariant())
            {
                case "word":
                    return ".docx";
                case "excel":
        ...
        public static async Task<(BitmapImage Image, Uri uri)> GetImageFromCache(string fileName)
        {
            try
            {
                var folder = ApplicationData.Current.LocalFolder;
